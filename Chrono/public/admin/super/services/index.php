...
// Build query – no admin_id condition
...
$pageTitle = 'Manage Services (Super)';
...
<div class="mb-6 flex justify-between items-center">
    <div>
        ...
        <p class="text-gray-500 dark:text-gray-400 mt-1">Manage platform services.</p>
    </div>
    <a href="<?= BASE_URL ?>/admin/super/services/add.php" class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg transition-all-200">
...
<form method="get" class="mb-4 flex flex-wrap gap-2">
    ...
    <a href="<?= BASE_URL ?>/admin/super/services/index.php" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-all-200">Clear</a>
</form>
        ...
        <table class="w-full">
            <thead class="bg-gray-50 dark:bg-slate-700 border-b border-gray-200 dark:border-gray-600">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Est. Price</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="table-row-hover">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white"><?= htmlspecialchars($service['name']) ?></td>
                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400"><?= htmlspecialchars(substr($service['description'] ?? '', 0, 50)) ?>...</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                        <?= $service['estimated_price'] ? '₱'.number_format($service['estimated_price'],2) : '-' ?>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 py-1 text-xs font-medium rounded-full 
                            <?= $service['status'] === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 
                                ($service['status'] === 'inactive' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' : 
                                'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400') ?>">
                            <?= ucfirst($service['status']) ?>
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <a href="<?= BASE_URL ?>/admin/super/services/edit.php?id=<?= $service['id'] ?>" class="text-primary-600 hover:text-primary-900 dark:text-primary-400 dark:hover:text-primary-300 mr-3">
                            <i class="fas fa-edit"></i> Edit
                        </a>
                        <?php if ($service['status'] !== 'deleted'): ?>
                        <a href="<?= BASE_URL ?>/admin/super/services/delete.php?id=<?= $service['id'] ?>&token=<?= urlencode($csrfToken) ?>" 
