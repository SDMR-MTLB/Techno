...
if (!$id) {
    header('Location: ' . BASE_URL . '/admin/super/offices/index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute([$name, $address, $city, $contact, $is_active, $id])) {
            header('Location: ' . BASE_URL . '/admin/super/offices/index.php?updated=1');
            exit;
        } else {
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold">Edit Office</h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1">Update office details.</p>
    </div>
        ...
        <form method="post">
            ...
            <div class="mb-4">
                ...
                <textarea id="address" name="address" required rows="2"
                          class="w-full px-3 py-2 bg-gray-50 dark:bg-slate-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all-200"><?= htmlspecialchars($_POST['address'] ?? $office['address']) ?></textarea>
            </div>
            ...
            <div class="mb-4">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" <?= (($_POST['is_active'] ?? $office['is_active']) == 1) ? 'checked' : '' ?> class="rounded border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-slate-700 text-primary-600">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition-all-200">Update Office</button>
            </div>
        </form>
    </div>
</div>
