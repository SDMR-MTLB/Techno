...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($check->fetch()) {
            ...
            $stmt = $db->prepare("INSERT INTO admins (username, password_hash, role) VALUES (?, ?, 'partner')");
            if ($stmt->execute([$username, $hash])) {
                header('Location: ' . BASE_URL . '/admin/super/admins.php?added=1');
                exit;
            } else {
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold">Add New Partner Admin</h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1">Create a new partner store account.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <form method="post">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition-all-200">Create Partner Admin</button>
            </div>
        </form>
    </div>
</div>
