...
$adminId = $_SESSION['admin_id']; // super admin's own ID
...
$sql = "SELECT * FROM packages WHERE admin_id = ?";
$params = [$adminId];
...
$countSql = "SELECT COUNT(*) FROM packages WHERE admin_id = ?";
$countParams = [$adminId];
if ($search) {
...
$pageTitle = 'Manage Packages (Super)';
...
<div class="mb-6 flex justify-between items-center">
    <div>
        ...
        <p class="text-gray-500 dark:text-gray-400 mt-1">Manage your packages.</p>
    </div>
    <a href="<?= BASE_URL ?>/admin/super/packages/add.php" class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg transition-all-200">
...
<form method="get" class="mb-4 flex flex-wrap gap-2">
    ...
    <a href="<?= BASE_URL ?>/admin/super/packages/index.php" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-all-200">Clear</a>
</form>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="table-row-hover">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400"><?= htmlspecialchars(substr($pkg['description'] ?? '', 0, 50)) ?>...</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">₱<?= number_format($pkg['price'],2) ?></td>
                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400"><?= htmlspecialchars(substr($pkg['inclusions'] ?? '', 0, 30)) ?>...</td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <a href="<?= BASE_URL ?>/admin/super/packages/edit.php?id=<?= $pkg['id'] ?>" class="text-primary-600 hover:text-primary-900 dark:text-primary-400 dark:hover:text-primary-300 mr-3">
                            <i class="fas fa-edit"></i> Edit
                        </a>
                        <?php if ($pkg['status'] !== 'deleted'): ?>
                        <a href="<?= BASE_URL ?>/admin/super/packages/delete.php?id=<?= $pkg['id'] ?>&token=<?= urlencode($csrfToken) ?>" 
